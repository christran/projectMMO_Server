...
module.exports = function(io, socket, clients) {
    socket.on('login', async (data, callback) => {
        let account = await Account.getAccount(data.username).catch((err) => console.log(`[Login Server] Login | Error: ${err}`));
    ...
    socket.on('createCharacter', async (data, callback) => {
        ...
        try {
            let saveChar = await newChar.save();
    ...
    socket.on('getCharacters', async (data, callback) => {
        let characters = await Account.getCharacters(data.accountID).catch((err) => console.log(`[Login Server] getCharacters | Error: ${err}`));

        callback(_.keyBy(characters, 'name'));
    });
    
    socket.on('selectCharacter', async (data) => {
        let character = await Character.getCharacterByID(data._id).catch((err) => console.log(`[Login Server] selectCharacter | Error: ${err}`));
